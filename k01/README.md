...
- 入出力結果を貼り付けてください. 
- プロトタイプ宣言の最後には `;` が必要です. 
- プログラムを見るかぎり, varやaveなどの変数はグローバル宣言する必要はなさそうです. main内で宣言しましょう. 
- 実行するとエラーが出て, きちんとした結果が出ませんでした. プログラムを修正してください. 
    - 例えば, var_online関数を呼び出すときに引数として`square_ave`を渡していますが, square_aveを求める計算をしていない(square_aveにちゃんとした値が入っていない)ので上手くいきません. 
